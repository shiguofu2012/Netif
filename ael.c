...
// wait for 100 ms for device to get out of reset.
...
// wait for 50ms for datapath reset to complete. (mandatory patch)

/*
 * Transceiver-specific MDIO patches
 */
const u16 trans_mdio_patch[] = {
		0xc003, 0x181,  // (bit 7) enable the CDR inc setting in 1.C005 (mandatory patch for SR code)
		0xc010, 0x448a // (bit 14) mask out high BER input from the LOS signal in 1.000A (mandatory patch for SR code)
};

const u16 act_uc_pause[] = {
...
/*
 * unpuase microcontroller and start program
 */
const u16 start_uc_prog[] = {
		0xca00, 0x0080,
		0xca12, 0x0000
};
...
int ael2005_initialize(XEmacLite *InstancePtr, u32 dev){

        int size, i;
        unsigned int value;
		for(i = 20; i < 36; i++){
		     ael2005_i2c_read (InstancePtr, dev, MODULE_DEV_ADDR, i, &value);
		     xil_printf("%c", value);
	    }
		for(i = 40; i < 56; i++){
		     ael2005_i2c_read (InstancePtr, dev, MODULE_DEV_ADDR, i, &value);
        ...
        // Step 1
        print("Step 1 - Software Reset..\r\n");
        size = sizeof(regs0) / sizeof(u16);
        for(i = 0; i < size; i+=2) ael2005_write(InstancePtr, dev, 1, sw_rst[i], sw_rst[i+1]);
        ael2005_sleep(100); // wait for 100 ms for device get out of reset
        ...
        for(i = 0; i < size; i+=2) ael2005_write(InstancePtr, dev, 1, start_uc_prog[i], start_uc_prog[i+1]);
        print("AEL2005 Initialization is Done\r\n");
        ael2005_sleep(200);
        return XST_SUCCESS;
}
